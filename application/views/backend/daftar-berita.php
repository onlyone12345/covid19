...
<div class="content-wrapper">
				...
				<div class="box box-info">
					<div class="box-header with-border">
						...
						<div class="pull-right">
							<a href="<?php echo site_url() ?>backend/berita/tulis-berita" type="button" class="btn btn-success button-sm btn-flat"><i class="fa fa-plus"></i> Tambah Berita</a>
						</div>
					</div>
					<!-- /.box-header -->
					<div class="box-body">
	...
	<div class="modal fade modal" id="modal-publish">
		<div class="modal-dialog ">
		<div class="modal-content">
			<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			...
			<div class="modal-body">
			<b><i>Konfirmasi Publikasi Berita ?</i></b>
			<form class="confirm-publish" action="<?php echo site_url() ?>backend/berita/publish" method="POST">
				<input type="hidden" name="id" >
			</div>
			<div class="modal-footer">
				<button type="submit" class="btn btn-success btn-flat"><i class="fa fa-trash"></i> ya, Publikaskan</button>
			</div>
			</form>	
		</div>
		<!-- /.modal-content -->
	...
	<div class="modal fade modal" id="modal-un-publish">
		<div class="modal-dialog ">
		<div class="modal-content">
			<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			...
			<div class="modal-body">
			<b><i>Konfirmasi Unpublikasi Berita ?</i></b>
			<form class="confirm-publish" action="<?php echo site_url() ?>backend/berita/publish" method="POST">
				<input type="hidden" name="id" >
			</div>
			<div class="modal-footer">
				<button type="submit" class="btn btn-danger btn-flat"><i class="fa fa-trash"></i> ya, Unpublikaskan</button>
			</div>
			</form>	
		</div>
		<!-- /.modal-content -->
...
<script>
	$(document).ready(function() {
		...
		$('#datatable_custom tbody').on('click', '.publish', function(){
			var id = $(this).attr('data-id');
			$('#modal-publish input[name=id]').val(id);
			console.log(id);
			$('#modal-publish').modal('toggle');
		});

		$('#datatable_custom tbody').on('click', '.un-publish', function(){
			var id = $(this).attr('data-id');
			$('#modal-un-publish input[name=id]').val(id);
			console.log(id);
			...
			$.ajax({
				...
				success: function(response) {
					...
					if ( response.status == "success" ){
						...
						$('#modal-delete, #modal-publish, #modal-un-publish').modal('hide');
